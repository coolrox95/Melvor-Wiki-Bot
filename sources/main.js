...
var menuDivD = document.createElement('div');
...
var menuDivC = document.createElement('div');
...
var items, MONSTERS, DUNGEONS, combatAreas, SPELLS, tiers, cookingFireData, itemUses, masterTable, numberMultiplier, miningData, smithingItems, runecraftingItems, axeLevels, rodLevels, pickaxeLevels, gloveID, skillcapeItems, baseMiningInterval, axeCost, rodCost, pickaxeCost, glovesCost, runecraftInterval, axeBonusSpeed, rodBonusSpeed, pickaxeBonus;
var pickaxeBonusSpeed, logsData, PRAYER, slayerItems, herbloreItemData, skillName, newFarmingAreas, autoEatData, smithInterval, fletchInterval, craftInterval, herbloreInterval, fishingItems, junkItems, specialItems, fishingAreas, enemySpecialAttacks;
var openableItems = [];
var slayerRangedArmour, slayerMeleeArmour, slayerMagicArmour, wikiPageNames, disambiguationData, slayerAreas, godUpgradeData, godDungeonID;
var godUpgradeDescriptions = [
...
function processWikiData() {
    try {
        ...
        for (let i = 0; i < fishingItems.length; i++) {
            let itemID = fishingItems[i].itemID;
            items[itemID].creationSources.push({
                skill: 'Fishing',
        ...
        for (let i = 0; i < combatAreas.length; i++) {
            ...
            for (let j = 0; j < combatAreas[i].monsters.length; j++) {
                let monID = combatAreas[i].monsters[j];
        ...
        for (let i = 0; i < slayerAreas.length; i++) {
            ...
            for (let j = 0; j < slayerAreas[i].monsters.length; j++) {
                let monID = slayerAreas[i].monsters[j];
                if (MONSTERS[monID].lootTable != undefined) {
                    for (let k = 0; k < MONSTERS[monID].lootTable.length; k++) {
                        items[MONSTERS[monID].lootTable[k][0]].monsterSources.push(
                            {
        ...
        //Create pageNames
        let pageNameData = findDuplicatePageNames();
...
//Function to generate item uses
...
function getSimNames() {
    ...
    for (let i = 0; i < combatAreas.length; i++) {
        for (let j = 0; j < combatAreas[i].monsters.length; j++) {
            dataSet.push(MONSTERS[combatAreas[i].monsters[j]].name);
        }
    }
    //Compile data from monsters in slayer zones
    for (let i = 0; i < slayerAreas.length; i++) {
        for (let j = 0; j < slayerAreas[i].monsters.length; j++) {
            dataSet.push(MONSTERS[slayerAreas[i].monsters[j]].name);
        }
    }
    //Perform simulation of monsters in dungeons
    for (let i = 0; i < DUNGEONS.length; i++) {
        dataSet.push(DUNGEONS[i].name)
    }
    return dataSet;
}
