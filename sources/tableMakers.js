...
function createCookingTable() {
       ...
       cookSpec.appendColumn('Healing', 'right', 'cookedItemID', 0, (id) => { return formatAsInt(items[id].healsFor * numberMultiplier) })
       cookSpec.appendColumn('Price', 'right', 'cookedItemID', 0, formatItemIDasPrice)
       cookSpec.appendColumn('Ingredients', 'right', 'id', 0, (id) => { return `1 ${formatItemIDAsImageLink(id, 25, 'middle')} ${formatItemIDAsLink(id)}` })
...
function createFarmingPlotsTable(areaID) {
       var plotSpec = new tableSpecMaker();
...
function createFarmingHerbTable() {
       var farmableItems = getObjectArraySubset(items, selectHerbSeed)
       var farmSpec = new tableSpecMaker();
       farmSpec.appendColumn('Seeds', 'left', 'id', 0, id => formatItemIDAsImageLink(id, 50, 'middle'));
       farmSpec.appendColumn('Name', 'left', 'id', 0, formatItemIDAsLink);
       farmSpec.appendColumn('Farming Level', 'right', 'farmingLevel', 1, formatAsInt);
...
function createMiningTable() {
       ...
       tabSpec.appendColumn('Experience', 'right', 'ore', 0, id => { return formatAsInt(items[id].miningXP) });
       ...
       var miningData2 = getObjectArraySubset(miningData, () => { return true })
...
function createFishingTable() {
       ...
       tabSpec.appendColumn('Experience', 'right', 'itemID', 0, id => { return formatAsInt(items[id].fishingXP) });
...
function createFishingAreasTable() {
       ...
       tabSpec.appendColumn('Fish', 'right', 'fish', [], fish => {
              ...
              for (let i = 0; i < fish.length; i++) {
                     let itemID = fishingItems[fish[i]].itemID;
...
function createNonGearUpgradeTable() {
       var itemSubset = getObjectArraySubset(items, selectNonGearUpgradeable);
       var tabSpec = new tableSpecMaker();
       tabSpec.appendColumn('Item', 'left', 'id', '', id => formatItemIDAsImageLink(id, 50, 'middle'));
       tabSpec.appendColumn('Name', 'left', 'id', '', formatItemIDAsLink);
       tabSpec.appendColumn('Ingredients', 'right', 'trimmedItemID', 0, formatItemTrimCost);
...
function createMonsterTable() {
       ...
       */
       ...
       tabSpec.appendColumn('Coins', 'right', 'dropCoins', [0, 0], x => { return `${x[0]}-${x[1]}` }, x=>{return (x[0]+x[1])/2});
...
function createSmithingTable(type) {
       ...
       itemSubset.sort((a, b) => { return a.smithingLevel - b.smithingLevel });
       var tabSpec = new tableSpecMaker();
...
function createArmourTable(equipmentSlot, type, ammoType = -1) {
       ...
       var itemSubset = getObjectArraySubset(items, item => selectArmourItem(item, equipmentSlot, type, ammoType))
       if (type == 'Melee') {
              itemSubset.sort(sortByDefenceLevel)
       ...
       if (equipmentSlot == CONSTANTS.equipmentSlot.Cape || equipmentSlot == CONSTANTS.equipmentSlot.Ring || equipmentSlot == CONSTANTS.equipmentSlot.Amulet || (equipmentSlot == CONSTANTS.equipmentSlot.Gloves && type == 'None')) {
              ...
              tabSpec.appendColumn('style="padding:0 1em 0 0.5em;"|Description', 'right;padding: 0 0.5em 0 0.5em', 'description', 'None', a => { return a })
       }
       if (!isSkillItem) {
...
function createWeaponTable(type, ammoTypeRequired = -1) {
       var itemSubset = getObjectArraySubset(items, item => selectWeaponItem(item, type, ammoTypeRequired))
       if (type == 'Melee') {
              itemSubset.sort(sortByAttackLevel)
       } else if (type == 'Ranged') {
              itemSubset.sort(sortByRangedLevel)
...
function createShopCookingFireTable() {
       ...
       tabSpec.appendColumn('Cost', 'right', ['costGP', 'costLogs'], 0, x => { return `${formatItemIDAsImageLink(x[1][0], 25, 'middle')} ${x[1][1]}<br/>${formatAsShopCost(x[0])}` },x=>{return x[0]});
...
function createShopGodUpgradeTable() {
       ...
       tabSpec.appendColumn('Dungeon', 'right', 'dungeon', 0, x => {return `${formatDungeonIDAsImageLink(x,25,'middle')} ${formatDungeonIDAsLink(x)}`});
       tabSpec.appendColumn('Cost', 'right', 'cost', 0, formatAsShopCost,returnSelf);
...
function createShopGloveTable() {
       ...
       tabSpec.appendColumn('Cost', 'right', 'cost', 0, formatAsShopCost,returnSelf);
       tabSpec.appendColumn('Cost per Charge', 'right', ['cost', 'charges'], 0, x => formatAsShopCost(x[0] / x[1]),x=>{return x[0]/x[1]});
...
function createShopMaterialTable() {
       var materialIDs = [CONSTANTS.item.Compost, CONSTANTS.item.Weird_Gloop, CONSTANTS.item.Bowstring, CONSTANTS.item.Leather, CONSTANTS.item.Green_Dragonhide, CONSTANTS.item.Blue_Dragonhide, CONSTANTS.item.Red_Dragonhide];
       var itemSubset = [];
       materialIDs.forEach(x => { itemSubset.push(items[x]); itemSubset[itemSubset.length - 1].itemID = x; });
...
function createShopSkillcapeTable() {
       ...
       tabSpec.appendColumn('Name', 'left', 'itemID', 0, formatItemIDAsLink);
       tabSpec.appendColumn('Description', 'right', 'itemID', 0, x => { return items[x].description });
...
function createShopSlayerTable() {
       ...
       tabSpec.appendColumn('Item', 'left', 'itemID', 0, id => formatItemIDAsImageLink(id, 50, 'middle'));
       tabSpec.appendColumn('Name', 'left', 'itemID', 0, formatItemIDAsLink);
       tabSpec.appendColumn('Description', 'right', 'description', 'None', x => { return x });
...
function createCraftingTable(type) {
       var itemSubset = getObjectArraySubset(items, item => selectCraftingItem(item, type));
       itemSubset.sort((a, b) => { return a.craftingLevel - b.craftingLevel });
       var tabSpec = new tableSpecMaker();
       tabSpec.appendColumn('Item', 'left', 'id', 0, id => formatItemIDAsImageLink(id, 50, 'middle'));
       tabSpec.appendColumn('Name', 'left', 'id', 0, formatItemIDAsLink);
...
function createWoodCuttingTable() {
       var treeData = getObjectArraySubset(trees, () => { return true })
       var logSpec = new tableSpecMaker();
...
function createItemsTable() {
       var itemData = getObjectArraySubset(items, () => { return true })
       var tabSpec = new tableSpecMaker();
       tabSpec.appendColumn('Item', 'left', 'id', 0, id => formatItemIDAsImageLink(id, 50, 'middle'));
       tabSpec.appendColumn('Name', 'left', 'id', 0, formatItemIDAsLink);
       tabSpec.appendColumn('ID', 'right', 'parentIndex', 0, formatAsInt);
       tabSpec.appendColumn('Category', 'left', 'category', 'No Category', x => { return x });
       tabSpec.appendColumn('Type', 'left', 'type', 'No Type', x => { return x });
...
function createFletchingTable(type) {
       var fletchingItems = getObjectArraySubset(items, item => selectFletchingItem(item, type));
       fletchingItems.sort((a, b) => { return a.fletchingLevel - b.fletchingLevel; });
       var tabSpec = new tableSpecMaker();
       tabSpec.appendColumn('Item', 'left', 'id', 0, id => formatItemIDAsImageLink(id, 50, 'middle'));
       tabSpec.appendColumn('Name', 'left', 'id', 0, formatItemIDAsLink);
...
function createPrayerTable() {
       var prayerSubset = getObjectArraySubset(PRAYER, selectAll)
       var tabSpec = new tableSpecMaker();
       ...
       tabSpec.appendColumn('Point Cost', 'left', ['pointsPerEnemy', 'pointsPerPlayer', 'pointsPerRegen'], [0, 0, 0], formatPrayerCosts,x=>{
              let maxCost = 0;
              x.forEach(cost=>{if (cost > maxCost) {
                     maxCost = cost;
              }})
              return maxCost;
       });
...
function createBonesTable() {
       let bones = getObjectArraySubset(items, item => selectIfHasKey(item, 'prayerPoints'));
       bones.sort((a, b) => { return a.fletchingLevel - b.fletchingLevel; });
       let tabSpec = new tableSpecMaker();
...
function createXPTable() {
       let xpTable = [{ level: 1, xp: 0, xpToNext: 0 }];
       let xp = 0;
       for (let i = 1; i < 99; i++) {
...
function createPotionsTable(type) {
       ...
       itemSubset.sort((a, b) => { return a.herbloreLevel - b.herbloreLevel });
       var tabSpec = new tableSpecMaker();
       ...
       tabSpec.appendColumn('Effect', 'left', 'description', 'None', x => { return x });
...
function createItemLootSourcesTable(itemID) {
       ...
       tabSpec.appendColumn('Quantity', 'right', ['minQty', 'maxQty'], [1, 1], formatAsDropQty, x=>{return x[0]+x[1]});
