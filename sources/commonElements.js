...
/**
 * @description Creates a string arry of the items sources to be formatted later
 * @param {number} itemID Index of items array
 */
function getItemSourcesArray(itemID) {
    ...
    for (let i = 0; i < slayerAreas.length; i++) {
        for (let j = 0; j < slayerAreas[i].monsters.length; j++) {
            let monsterID = slayerAreas[i].monsters[j];
            if (MONSTERS[monsterID].lootTable != undefined) {
                for (let k = 0; k < MONSTERS[monsterID].lootTable.length; k++) {
                    if (MONSTERS[monsterID].lootTable[k][0] == itemID) {
                        monsterList += `${formatMonsterIDAsImageLink(monsterID, 25, 'middle')}, `;
    ...
    let shopItems = [CONSTANTS.item.Green_Dragonhide, CONSTANTS.item.Blue_Dragonhide, CONSTANTS.item.Red_Dragonhide, CONSTANTS.item.Cooking_Gloves, CONSTANTS.item.Mining_Gloves, CONSTANTS.item.Smithing_Gloves, CONSTANTS.item.Thieving_Gloves, CONSTANTS.item.Gem_Gloves, CONSTANTS.item.Bowstring, CONSTANTS.item.Compost, CONSTANTS.item.Leather, CONSTANTS.item.Weird_Gloop];
    let inShop = false;
...
function findDuplicatePageNames() {
    ...
    let dedupeStrings = { //Append to pageName to uniquely identify them
        items: ' (item)',
        monsters: ' (monster)',
    ...
    let pageNames = { //Arrays with unique page names to generate links/create pages with
        items: [],
        monsters: [],
        thievingTarget: [],
        combatAreas: [],
    ...
    let monsterDupes = findSelfDuplicateNames(MONSTERS, 'name');
    ...
    for (let i = 0; i < tiers.length; i++) {
        let axeName = getAxeUpgradeName(i);
        let pickName = getPickUpgradeName(i);
        let rodName = getRodUpgradeName(i);
    ...
    for (let i = 0; i < cookingFireData.length; i++) {
        let fireName = getFireUpgradeName(i);
    ...
    let miscUpgrades = ['Multi-Tree', 'Extra Equipment Set', 'Bank Slot'];
    ...
    for (let i = 0; i < MONSTERS.length; i++) {
        let dupeIndices = findItemOnArray(MONSTERS[i].name, monsterDupes.duplicateNames);
    ...
    Object.keys(pageNames).forEach(nameClass => {
        for (let i = 0; i < pageNames[nameClass].length; i++) {
            let dupeIndices = findItemOnArray(pageNames[nameClass][i], dupeData.duplicateNames);
            if (dupeIndices.length > 0) {
...
function replaceTableInString(originalString, newTableString) {
    //Replace the table in the original string with the newTable
    return originalString.replace(TABLEREGEX, newTableString)
}

function getFileExtension(filename) {
    let matches = filename.match(EXTENSIONREGEX);
