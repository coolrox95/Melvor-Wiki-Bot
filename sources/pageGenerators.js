...
function createItemPageContentOld(itemID) {
    let pageContent = '';
    pageContent += VERSIONTEMPLATE;
    pageContent += fillItemTemplate(itemID) + '\n';
    // pageContent += createItemDescription(itemID); Too difficult to do, I give up
    if (items[itemID].canOpen) {
        pageContent += createSection('Loot Table');
        pageContent += createChestDropTable(itemID) + '\n';
    }
    if (items[itemID].equipmentSlot != undefined) {
        if (items[itemID].equipmentSlot == 4) {
            pageContent += fillWeaponStatsTemplate(itemID) + '\n';
    ...
    //Creation Template
    if (items[itemID].creationSources.length > 0) {
        pageContent += createSection('Item Creation');
        for (i = 0; i < items[itemID].creationSources.length; i++) {
            pageContent += items[itemID].creationSources[i].fillTemplate(itemID) + '\n';
        }
    }
    //Loot sources Template
    if (items[itemID].hasLootSource) {
        pageContent += createSection('Loot Sources');
        pageContent += createItemLootSourcesTable(itemID) + '\n';
    }
    pageContent += '[[Category:Items]]\n';
    pageContent += '{{Menu}}';
    return pageContent;
...
function createThievingTargetPage(targetInd) {
    ...
    let orderNames = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth'];
    outputStr += '\n\n';
...
function createMonsterLootTablePage() {
    ...
    for (let i = 0; i < combatAreas.length; i++) {
        ...
        for (let j = 0; j < combatAreas[i].monsters.length; j++) {
            var lootChance = getMonsterLootChance(combatAreas[i].monsters[j])
    ...
    for (let i = 0; i < slayerAreas.length; i++) {
        ...
        for (let j = 0; j < slayerAreas[i].monsters.length; j++) {
            var lootChance = getMonsterLootChance(slayerAreas[i].monsters[j]);
...
function createUpgradedGearPage() {
    var outStr = '';
    var gearNames = Object.keys(CONSTANTS.equipmentSlot);
...
function createTableTemplatePage(tableText) {
    var outStr = `<noinclude>This template was autogenerated by MelvorWikiBot.\n${BOTCATEGORY}\n[[Category:Tables]]\n${VERSIONCATEGORY}</noinclude>\n`;
    outStr += tableText;
    return outStr;
}
